...
# ─────────────────────────────────────────────────────────────────────
# Linux-fast XML patch writer (no xlwings)
# ─────────────────────────────────────────────────────────────────────
...
def sanitize_xml_text(s) -> str:
    return _INVALID_XML_CHARS.sub("", "" if s is None else str(s))
...
def _clamp_coords(c1, r1, c2, r2, last_col, last_row):
    ...
    r1n = max(1, min(int(r1), last_row))
    r2n = max(1, min(int(r2), last_row))
...
def _clamp_defined_names(workbook_xml_bytes: bytes, sheet_name: str, last_col: int, last_row: int) -> bytes:
    try:
        ...
        def clamp_text(text: str) -> str:
            ...
            def repl_row(m):
                if _norm_sheet_name(m.group("sheet")).lower() != sheet_name.lower(): return m.group(0)
                r1 = max(1, min(int(m.group("r1")), last_row))
                r2 = max(1, min(int(m.group("r2")), last_row))
...
def _patch_sheet_xml(sheet_xml_bytes: bytes, header_row: int, start_row: int, used_cols_final: int, block_2d: list) -> bytes:
    ...
    # clamp data validations + fix count
    ...
    if dvs is not None:
        ...
        elif "count" in dvs.attrib:
            del dvs.attrib["count"]

    # clamp merged cells + fix count
    ...
    if merges is not None:
        ...
        for mc in list(merges):
            ...
            count += 1
        if count:
            merges.set("count", str(count))
        elif "count" in merges.attrib:
            del merges.attrib["count"]

    # clamp sheetViews selection (activeCell/sqref) to new bounds
    sheetViews = root.find(f"{{{XL_NS_MAIN}}}sheetViews")
    if sheetViews is not None:
        for sv in sheetViews.findall(f"{{{XL_NS_MAIN}}}sheetView"):
            for sel in sv.findall(f"{{{XL_NS_MAIN}}}selection"):
                ac = sel.attrib.get("activeCell")
                if ac:
                    m = CELL_RE.match(ac)
                    if m:
...
def fast_patch_template(master_bytes: bytes, sheet_name: str, header_row: int, start_row: int, used_cols: int, block_2d: list) -> bytes:
    ...
    - Writes values as inline strings (fast).
    - Clamps AutoFilter/CF/DataValidations/merges/sheetViews/dimension.
    ...
    # final write width = min(used_cols, min(table widths))
