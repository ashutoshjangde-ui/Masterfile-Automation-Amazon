...
# Namespaces (we DON'T add x14ac attributes anymore)
...
def sanitize_xml_text(s): return _INVALID_XML_CHARS.sub("", "" if s is None else str(s))
...
def _parse_range(ref):
    ...
    return (("A",1,"A",1) if not m else (m.group(1), int(m.group(2)), m.group(3), int(m.group(4))))

def _norm_sheet_name(n): 
    n=n.strip(); 
    return n[1:-1] if (n.startswith("'") and n.endswith("'")) else n

def _find_sheet_part_path(z, sheet_name):
    ...
    if not target: raise ValueError("Sheet relationship not found")
    target=target.replace("\\","/")
    if target.startswith("../"): target=target[3:]
    if not target.startswith("xl/"): target="xl/"+target
    return target

def _get_table_paths_for_sheet(z, sheet_path):
...
def _read_table_info(table_xml_bytes):
    root = ET.fromstring(table_xml_bytes)
    sc,sr,ec,er = _parse_range(root.attrib.get("ref","A1:A1"))
...
def _union_dimension(orig_dim_ref, used_cols, last_row):
    try:
        _, right = (orig_dim_ref or "A1:A1").split(":",1)
        m = CELL_RE.match(right)
        orig_last_col = _col_number(m.group(1)) if m else used_cols
        orig_last_row = int(m.group(2)) if m else last_row
    except Exception:
        orig_last_col, orig_last_row = used_cols, last_row
    return f"A1:{_col_letter(max(orig_last_col, used_cols))}{max(orig_last_row, last_row)}"

def _clamp_coords(c1,r1,c2,r2,last_col,last_row):
    c1n=max(1,min(_col_number(c1),last_col)); c2n=max(1,min(_col_number(c2),last_col))
    r1n=max(1,min(int(r1),last_row));       r2n=max(1,min(int(r2),last_row))
    if c2n<c1n: c1n,c2n=c2n,c1n
    if r2n<r1n: r1n,r2n=r2n,r1n
    return _col_letter(c1n), r1n, _col_letter(c2n), r2n

def _clamp_sqref_list(sqref,last_col,last_row):
    if not sqref: return sqref
    out=[]
...
def _clamp_ref(ref,last_col,last_row):
    m=RANGE_RE.match(ref or "")
    if not m: return ref
    c1,r1,c2,r2=m.group(1),int(m.group(2)),m.group(3),int(m.group(4))
    c1L,r1N,c2L,r2N=_clamp_coords(c1,r1,c2,r2,last_col,last_row)
...
def _clamp_defined_names(workbook_xml_bytes,sheet_name,last_col,last_row):
    try:
        ...
        def clamp_text(t):
            if not t: return t
            def ra(m):
                if _norm_sheet_name(m.group("sheet")).lower()!=sheet_name.lower(): return m.group(0)
                c1,r1,c2,r2=m.group("c1"),m.group("r1"),m.group("c2"),m.group("r2")
                c1L,r1N,c2L,r2N=_clamp_coords(c1,r1,c2,r2,last_col,last_row)
                return f"{m.group('sheet')}!${c1L}${r1N}:${c2L}${r2N}"
            t = SHEET_A1_RE.sub(ra,t)
            def rc(m):
                if _norm_sheet_name(m.group("sheet")).lower()!=sheet_name.lower(): return m.group(0)
                c1n=max(1,min(_col_number(m.group('c1')),last_col))
                c2n=max(1,min(_col_number(m.group('c2')),last_col))
                if c2n<c1n: c1n,c2n=c2n,c1n
                return f"{m.group('sheet')}!${_col_letter(c1n)}:${_col_letter(c2n)}"
            t = SHEET_COL_ONLY_RE.sub(rc,t)
            def rr(m):
                if _norm_sheet_name(m.group("sheet")).lower()!=sheet_name.lower(): return m.group(0)
                r1=max(1,min(int(m.group('r1')),last_row)); r2=max(1,min(int(m.group('r2')),last_row))
                if r2<r1: r1,r2=r2,r1
                return f"{m.group('sheet')}!${r1}:${r2}"
            return SHEET_ROW_ONLY_RE.sub(rr,t)
        for dn in list(dnames): dn.text = clamp_text(dn.text or "")
        return ET.tostring(root,encoding="utf-8",xml_declaration=True)
    except Exception:
        return workbook_xml_bytes

def _strip_calcchain_from_content_types(ct_bytes):
    try:
        root=ET.fromstring(ct_bytes)
        for ch in list(root):
            if ch.tag.endswith("Override") and ch.attrib.get("PartName")=="/xl/calcChain.xml":
                root.remove(ch)
        return ET.tostring(root,encoding="utf-8",xml_declaration=True)
    except Exception:
        return ct_bytes
...
def fast_patch_template(master_bytes, sheet_name, header_row, start_row, used_cols, block_2d):
    ...
    new_sheet_xml=_patch_sheet_xml(zin.read(sheet_path), header_row, start_row, final_cols, block_2d)
    last_row=max(header_row, start_row+max(0,len(block_2d)-1))

    patched_tables={}
    for (tp,sc,sr,width) in table_infos:
        try: patched_tables[tp]=_patch_table_xml(zin.read(tp), header_row, last_row, sc, width)
        except Exception: pass

    # clamp defined names + drop calcChain + scrub content-types entry
    ...
    with zipfile.ZipFile(out_bio,"w",zipfile.ZIP_DEFLATED) as zout:
        for item in zin.infolist():
            if item.filename=="xl/calcChain.xml":  # drop stale chain
                continue
            data=zin.read(item.filename)
            if item.filename==sheet_path: data=new_sheet_xml
            elif item.filename in patched_tables: data=patched_tables[item.filename]
            elif item.filename=="xl/workbook.xml": data=new_workbook_xml
            elif item.filename=="[Content_Types].xml": data=_strip_calcchain_from_content_types(data)
...
.section{border:1px solid var(--card-border);background:#fff;border-radius:16px;padding:18px 20px;box-shadow:0 6px 24px rgba(2,6,23,.05);margin-bottom:18px}
...
MASTER_TEMPLATE_SHEET="Template"; MASTER_DISPLAY_ROW=2; MASTER_SECONDARY_ROW=3; MASTER_DATA_START_ROW=4
...
def norm(s):
    ...
    x=re.sub(r"[._/\\-]+"," ",x); x=re.sub(r"[^0-9a-z\s]+"," ",x)
    return re.sub(r"\s+"," ",x).strip()

def top_matches(q,cands,k=3):
    qn=norm(q); scored=[(SequenceMatcher(None,qn,norm(c)).ratio(),c) for c in cands]
    scored.sort(key=lambda t:t[0],reverse=True); return scored[:k]

def worksheet_used_cols(ws, header_rows=(1,), hard_cap=2048, empty_streak_stop=8):
    max_try=min(ws.max_column,hard_cap); last_nonempty,streak=0,0
    for c in range(1,max_try+1):
        if any((ws.cell(row=r,column=c).value not in (None,"")) for r in header_rows):
...
def nonempty_rows(df): 
    return 0 if df.empty else df.replace("",pd.NA).dropna(how="all").shape[0]

def pick_best_onboarding_sheet(uploaded_file, mapping_aliases_by_master):
    uploaded_file.seek(0); xl=pd.ExcelFile(uploaded_file)
    ...
    for sheet in xl.sheet_names:
        ...
        matches=sum(any(norm(a) in header_set for a in aliases) for aliases in mapping_aliases_by_master.values())
        rows=nonempty_rows(df); score=matches+(0.01 if rows>0 else 0.0)
        if score>best_score: best,(best_score)=( (df,sheet), score ); best_info=f"matched headers: {matches}, non-empty rows: {rows}"
    if best is None: raise ValueError("No readable onboarding sheet found.")
...
with c1: masterfile_file=st.file_uploader("📄 Masterfile Template (.xlsx / .xlsm)", type=["xlsx","xlsm"])
with c2: onboarding_file=st.file_uploader("🧾 Onboarding (.xlsx)", type=["xlsx"])
...
if go:
    st.markdown("<div class='section'>", unsafe_allow_html=True)
    st.markdown("### 📝 Log"); log=st.empty()
    def slog(msg): log.markdown(msg)

    if not masterfile_file or not onboarding_file:
        st.error("Please upload both Masterfile Template and Onboarding."); st.markdown("</div>", unsafe_allow_html=True); st.stop()
    ...
    try:
        ...
        st.error(f"Mapping JSON parse error: {e}"); st.markdown("</div>", unsafe_allow_html=True); st.stop()
    ...
    slog("⏳ Reading Template headers…"); t0=time.time()
    wb_ro=load_workbook(io.BytesIO(master_bytes), read_only=True, data_only=True, keep_links=True)
    if MASTER_TEMPLATE_SHEET not in wb_ro.sheetnames:
        st.error(f"Sheet '{MASTER_TEMPLATE_SHEET}' not found."); st.markdown("</div>", unsafe_allow_html=True); st.stop()
    ...
    wb_ro.close(); slog(f"✅ Template headers loaded (cols={used_cols}) in {time.time()-t0:.2f}s")

    try:
        best_df,best_sheet,info=pick_best_onboarding_sheet(onboarding_file,mapping_aliases)
    except Exception as e:
        st.error(f"Onboarding error: {e}"); st.markdown("</div>", unsafe_allow_html=True); st.stop()
    on_df=best_df.fillna(""); on_df.columns=[str(c).strip() for c in on_df.columns]
    on_headers=list(on_df.columns); st.success(f"Using onboarding sheet: **{best_sheet}** ({info})")

    series_by_alias={norm(h): on_df[h] for h in on_headers}
    master_to_source, report_lines = {}, ["#### 🔎 Mapping Summary (Template)"]
    ...
    for c,(disp,sec) in enumerate(zip(display_headers,secondary_headers),start=1):
        ...
        if disp_norm==BULLET_DISP_N and sec_norm:
            effective_header=sec; label=f"{disp} ({sec})"
        else:
            effective_header=disp; label=disp
        eff_norm=norm(effective_header)
        if not eff_norm: continue
        aliases=mapping_aliases.get(eff_norm,[effective_header]); resolved=None
        for a in aliases:
            s=series_by_alias.get(norm(a))
            if s is not None: resolved=s; report_lines.append(f"- ✅ **{label}** ← `{a}`"); break
        if resolved is not None:
            master_to_source[c]=resolved
        else:
            if disp_norm==norm("Listing Action (List or Unlist)"):
                master_to_source[c]=SENTINEL_LIST; report_lines.append(f"- 🟨 **{label}** ← (will fill 'List')")
            else:
    ...
    for col,src in master_to_source.items():
        if src is SENTINEL_LIST:
            ...
            for i in range(m):
                ...
                if v and v.lower() not in ("nan","none",""): block[i][col-1]=v

    slog("🚀 Writing via Linux-fast XML (tables+names+CF/DV/merges; drop calcChain)…"); t_write=time.time()
    out_bytes=fast_patch_template(master_bytes, MASTER_TEMPLATE_SHEET, MASTER_DISPLAY_ROW, MASTER_DATA_START_ROW, used_cols, block)
    slog(f"✅ Wrote in {time.time()-t_write:.2f}s")

    st.download_button("⬇️ Download Final Masterfile", data=out_bytes, file_name=f"final_masterfile{ext}",
                       mime=out_mime, key="dl_xmlfast")
    st.markdown("</div>", unsafe_allow_html=True)

with st.expander("📘 How to use", expanded=False):
    st.markdown(dedent(f"""
    - Writes only into `{MASTER_TEMPLATE_SHEET}`; preserves other tabs, styles, formulas, macros.
    - Auto-syncs Table, AutoFilter, Conditional Formatting, Data Validations, merged cells, and Defined Names.
