...
# ─────────────────────────────────────────────────────────────────────
# FAST XML PATCH WRITER — preserves other sheets/styles/macros
# ─────────────────────────────────────────────────────────────────────
...
LOCAL_RANGE_RE = re.compile(r"\$(?P<c1>[A-Z]+)\$(?P<r1>\d+):\$(?P<c2>[A-Z]+)\$(?P<r2>\d+)", re.I)
...
def _find_sheet_part_path(z: zipfile.ZipFile, sheet_name: str) -> str:
    ...
    for idx, sh in enumerate(list(sheets_el)):
...
def _clamp(c1, r1, c2, r2, last_col, last_row):
...
def _clamp_sqref_list(sqref: str, last_col: int, last_row: int) -> str:
    ...
    for p in parts:
        ...
        c1L,r1N,c2L,r2N = _clamp(c1,r1,c2,r2,last_col,last_row)
...
def _patch_sheet_xml(sheet_xml_bytes: bytes, start_row: int, used_cols_final: int, block_2d: list) -> bytes:
    ...
    # remove old rows at/after start_row
    ...
    dv_parent = root.find(f"{{{XL_NS_MAIN}}}dataValidations")
    if dv_parent is not None:
        for dv in dv_parent.findall(f"{{{XL_NS_MAIN}}}dataValidation"):
    ...
    if merges is not None:
        for mc in list(merges):
            ...
            if not m: continue
            c1, r1, c2, r2 = m.group(1), int(m.group(2)), m.group(3), int(m.group(4))
            c1L,r1N,c2L,r2N = _clamp(c1,r1,c2,r2,last_col_num,last_row)
...
def _clamp_defined_names(workbook_xml_bytes: bytes, sheet_name: str, sheet_index0: int, last_col: int, last_row: int) -> bytes:
    """Clamp defined names that point into Template so they don't exceed new dimension."""
    try:
        ...
        def clamp_formula(text: str, is_local_to_template: bool) -> str:
            if not text: return text

            # 1) clamp occurrences that explicitly reference 'Template'!
            def repl_sheet(m):
            ...
            new_text = SHEET_RANGE_RE.sub(repl_sheet, text)

            # 2) if name is local to Template sheet and formula contains bare A1 ranges, clamp them too
            if is_local_to_template:
                def repl_local(m):
                    c1,r1,c2,r2 = m.group("c1"), m.group("r1"), m.group("c2"), m.group("r2")
                    c1L,r1N,c2L,r2N = _clamp(c1,r1,c2,r2,last_col,last_row)
                    return f"${c1L}${r1N}:${c2L}${r2N}"
                new_text = LOCAL_RANGE_RE.sub(repl_local, new_text)

            return new_text

        # find Template sheet index (0-based) for localSheetId match
        sheets_parent = root.find(f"{{{XL_NS_MAIN}}}sheets")
        template_local_idx = sheet_index0

        for dn in list(dnames):
            txt = dn.text or ""
            is_local = str(dn.attrib.get("localSheetId", "")) == str(template_local_idx)
            new_txt = clamp_formula(txt, is_local_to_template=is_local)
            dn.text = new_txt
...
def fast_patch_template(master_bytes: bytes, sheet_name: str, header_row: int, start_row: int, used_cols: int, block_2d: list) -> bytes:
    """Replace Template rows, sync tables, clamp CF/DV/merges, fix Defined Names, drop calcChain."""
    zin = zipfile.ZipFile(io.BytesIO(master_bytes), "r")
    workbook_xml = zin.read("xl/workbook.xml")
    wb_root = ET.fromstring(workbook_xml)

    # Sheet part path + index
    ...
    for idx, sh in enumerate(list(sheets_el)):
        if sh.attrib.get("name") == sheet_name:
            ...
            rid = sh.attrib.get(f"{{{XL_NS_REL}}}id")
            break
    if template_idx0 is None or not rid:
        raise ValueError(f"Sheet '{sheet_name}' not found")
    ...
    new_workbook_xml = _clamp_defined_names(workbook_xml, sheet_name, template_idx0, final_cols, last_row)

    out_bio = io.BytesIO()
    with zipfile.ZipFile(out_bio, "w", zipfile.ZIP_DEFLATED) as zout:
        for item in zin.infolist():
            # Drop calcChain so Excel rebuilds it (avoids “problem with content” dialog)
...
zip_wrapper = st.checkbox("Download as ZIP wrapper (can reduce Protected View on some machines)", value=False)
...
if go:
    ...
    # Patch
    slog("🚀 Writing via XML patch (tables + CF/DV + Defined Names + calcChain strip)…")
    t_write = time.time()
    ...
    # Download (xlsx/xlsm or zip wrapper)
    file_name = f"final_masterfile{ext}"
    if not zip_wrapper:
        st.download_button("⬇️ Download Final Masterfile", data=out_bytes, file_name=file_name, mime=out_mime, key="dl_xmlfast")
    else:
        bio = io.BytesIO()
        with zipfile.ZipFile(bio, "w", zipfile.ZIP_DEFLATED) as z:
            z.writestr(file_name, out_bytes)
        bio.seek(0)
        st.download_button("⬇️ Download ZIP (contains Excel file)", data=bio.getvalue(),
                           file_name=file_name.replace(ext, ".zip"), mime="application/zip", key="dl_zip")

    st.markdown("</div>", unsafe_allow_html=True)

with st.expander("📘 Notes", expanded=False):
    st.markdown(dedent(f"""
    - Writes only into `{MASTER_TEMPLATE_SHEET}` and preserves all other tabs, styles, formulas, and macros.
    - Tables, autoFilter, conditional formatting, data validation, merges, and Defined Names are synced/clamped.
    - `calcChain.xml` is removed so Excel rebuilds dependencies on open **without** the repair dialogs.
    - Protected View for Internet downloads is an Office/Windows security feature; code cannot disable it.
    """))
